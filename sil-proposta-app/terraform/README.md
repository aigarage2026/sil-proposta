# Terraform — Sil-Proposta Cloud Run

Skeleton infrastructure for deploying `sil-proposta-app/backend` to GCP
...
2. **Secrets in Secret Manager** (names match `variables.tf`):
   - `sil-proposta-database-url`
   - `sil-proposta-jwt-secret`
   - `sil-proposta-portal-hmac-secret`
   - `sil-proposta-openai-api-key` (optional — keep blank if demo-only)
   - `sil-proposta-anthropic-api-key` (optional)
3. **Terraform state bucket**: `gs://${project_id}-terraform-state`.

## Typical flow

```
cd sil-proposta-app/terraform
