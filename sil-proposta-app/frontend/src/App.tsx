...
import { silPropostaRoutes } from '@/products/sil-proposta/routes'
    ...
    <Suspense fallback={<RouteFallback />}>
      <Routes>
        ...
        <Route element={<ProtectedRoute><AppLayout /></ProtectedRoute>}>
          {/* sil-proposta product routes — see frontend/src/products/sil-proposta/routes.tsx.
              The Portal SPA will mount this set under /sil-proposta/* once that
              integration lands (Onda 6). Today it mounts at the SPA root. */}
          {silPropostaRoutes.map((r) => (
            <Route
              key={r.path ?? '_index'}
              index={r.index}
              path={r.path}
