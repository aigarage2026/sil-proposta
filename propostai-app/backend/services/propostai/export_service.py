...
def _render(sections: dict[str, Any]) -> io.BytesIO:
    ...
    run = p.add_run("DIRETO AO PONTO PARTNER")
    run.bold = True
    run.font.size = Pt(18)
    ...
    cover_rows = [
        ...
        ("Solicitante", "", None),
        ("", "", None),
        ("Preenchimento Direto ao Ponto", "", HEADER_BG_HEX),
        ("Data criação", data_hoje, None),
        ("Arquiteto", "PropostAI (IA)", None),
        ("Código PMS", "OP —", None),
    ]
    ...
    _body(
        doc,
        "É importante destacar que, conforme o modelo de Sustentação da Direto ao Ponto, "
...
# ════════════════════════════════════════════════════════════════════════════
# WP (Excel Work Package) — Direto ao Ponto template
# ════════════════════════════════════════════════════════════════════════════
#
# Migrated from legacy/propostai-monolith/backend/generators/wp.py.
...
def generate_wp_workbook(proposal) -> io.BytesIO:
    """Build the Direto ao Ponto Work Package .xlsx for a Proposal."""
...
def _render_wp(resources: list[dict[str, Any]], presale_hours: int) -> io.BytesIO:
    ...
    ws.cell(row=1, column=1, value="WORK PACKAGE — PROPOSTAI").font = WP_FONT_TITLE
