"""
Seed de dados iniciais da plataforma PropostAI.
...
KNOWLEDGE_DOCS = [
    ...
    {"title": "PS — Estrutura Padrao Direto ao Ponto", "category": "template",
     "content": "PS (Proposta de Solucao): 1.Necessidade, 2.Solucao, 3.Premissas, 4.Equipe, 5.Impactos, 6.Cronograma, 7.Investimento, 8.Condicoes."},
    {"title": "Modelo Comercial Direto ao Ponto", "category": "template",
...
async def seed():
    ...
    async with async_session() as db:
        ...
        result = await db.execute(select(Tenant).where(Tenant.slug == "direto-ao-ponto-demo"))
        ...
        # Tenant demo
        tenant = Tenant(
            name="Direto ao Ponto (Demo)",
            slug="direto-ao-ponto-demo",
        ...
        owner = User(
            tenant_id=tenant.id,
            email="admin@direto-ao-ponto.demo",
            full_name="Administrador Direto ao Ponto",
            password_hash=hash_password("DemoPropostAI2026!"),
            role="owner",
            is_active=True,
        ...
        # Knowledge docs
        from models.propostai.knowledge_document import KnowledgeDocument
