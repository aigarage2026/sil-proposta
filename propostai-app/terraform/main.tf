...
resource "google_artifact_registry_repository" "propostai" {
  location      = var.region
  repository_id = local.ar_repo
  format        = "DOCKER"
  description   = "PropostAI backend container images."
...
resource "google_cloud_run_v2_service" "propostai" {
  ...
  depends_on = [
    google_artifact_registry_repository.propostai,
...
resource "google_cloud_run_v2_service_iam_member" "public" {
  name     = google_cloud_run_v2_service.propostai.name
  location = google_cloud_run_v2_service.propostai.location
